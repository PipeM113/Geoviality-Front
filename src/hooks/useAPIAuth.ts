...
function useAPIAuth(
  ...
  apiDomain: string = `https://${process.env.REACT_APP_NGROK_DOMAIN}`
) {
  ...
  const verificarSesionAsync = async () => {
    ...
    const token = localStorage.getItem(astor_tokenName);
  ...
  const iniciarSesionAsync = async (username: string, password: string) => {
    ...
    try {
      ...
      // Se supone que la API responde con un token de sesión. Se guarda en localStorage y en el estado
      ...
      localStorage.setItem(astor_tokenName, newToken);
      localStorage.setItem(astor_username, username);
      return true;
    } catch (error) {
  ...
  const cerrarSesionAsync = async () => {
    console.log("useAPIAuth/cerrarSesion - Cerrando sesión... Borrando token de localStorage.");

    localStorage.removeItem(astor_tokenName);
    localStorage.removeItem(astor_username);
  };
